...
public function get_chapters()
{
    if ($_SESSION['role'] == "school")
    {
        $this->data['school'] = $this->configuration_m->get_multiple_row('school_registration',array('schoolID'=>$_SESSION['loginUserID'],'is_active'=>"Y"),'schoolID,name');
        ...
        $school_name = $this->db->get_where('school_registration',array('schoolID'=> $schoolID))->row();
        $sch_name = $school_name->name;
        $array1 = array('schoolID'=>$schoolID,'classID'=>$classID,'sectionID'=>$sectionID,'subjectID'=>$subjectID);
        $chapters = $this->configuration_m->get_multiple_row('chapters',$array1);
        if(!empty($chapters)){
            foreach($chapters as $chap){
                $this->db->select('subject_name');

                $subject = $this->db->get_where('subject',array('subjectID'=>$chap->subjectID))->row();

                $chap->subjectID = isset($subject->subject_name) ? $subject->subject_name :'';
...
public function get_chapters_by_id(){
    ...
    $d = $this->db->get_where('chapters',array('chapterID'=>$chapterID))->row();
    if ($d) {
...
//                        $this->db->update('subject',array('syllabus'=>$actual_image_name));
    ...
    public function edit_chapter()
    {
        if ($_SESSION['role'] == "school")
        {
            ...
            $this->db->update('chapters',array('classID'=>$classID,'chapter_name'=>$chapter_name,'is_active'=>$status));
            redirect(base_url('configuration/chapter'));
    ...
    public function add_chapter()
    {
        if ($_SESSION['role'] == "school")
        {
            ...
            foreach ($sectionID as $sectionID){
                ...
                $this->db->insert('chapters',array('schoolID'=>$schoolID,'classID'=>$classID,'sectionID'=>$sectionID,'subjectID'=>$subjectID,'chapter_name'=>$chapter_name,'is_active'=>$status,'added_on'=>date("Y-m-d H:i:s")));
            }
            redirect(base_url('configuration/chapter'));
