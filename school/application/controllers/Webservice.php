...
class Webservice extends CI_Controller {
    ...
    public function send_sms($mobile, $message){
        ...
        $msg = "sender=".$sender."&route=4&country=91&message=".$message."&mobiles=".$mobile."&authkey=285140ArLurg2KnR61e3d660P1";
    ...
    // }
    #Register
//     public function send_mail_old($email,$subject,$message){
//       $to = $email;
//       $from = 'anishashrivastava20@gmail.com';
    ...
    /* if (count($validate)) {*/
        if (!empty($validate)) {
            ...
            /*if (count($class_check) && count($section_check)) {*/
             if (!empty($class_check) && !empty($section_check)) {
...
public function login()
{
    ...
    if(!empty($check))
    {
        ...
        if($role == 'school' || $role == 'school_registration')
        {
            ...
            $get_school_config = $this->db->get_where('school_config',array('schoolID'=>$id))->result();
            ...
            /*if (count($get_school_config) && $check->is_config_setup == "Y") {*/
                if (!empty($get_school_config) && $check->is_config_setup == "Y") {
                    $config_status = "Y";
                }else{
                    $config_status = "N";
                }
...
//            $data1 = array("m" => $data1);
//            $this->fcmNotification($device_token, $data1);
            $response[] = array('result'=>'success','code'=>$check->school_code, 'schoolID'=>$check->schoolID,'id'=>$id, 'name'=>$check->name, 'email'=>$check->email, 'phone' => $check->phone, 'status'=>$status, 'image'=>base_url('uploads/images/').$img,'config_status'=>$config_status,'is_approved'=>$is_approved );
        } else {
            $response[] = array('result'=>'failure', 'message' => 'Invalid Details');
        }
        echo json_encode($response);
    ...
    public function student_home()
    {
        ...
        $check[] = $this->db->query("SELECT student.schoolID,COALESCE(NULLIF(student_group.latitude, ''),'') as latitude,COALESCE(NULLIF(student_group.longitude, ''),'') as longitude,student.classID,student.sectionID,classes.class as class_name,section.section,student.school_code,student.username,student.is_active,student.`name`,CONCAT('".$path."',COALESCE(NULLIF(student.image, ''),'default.png')) as image,GROUP_CONCAT(modules.module_name ORDER BY modules.module_name SEPARATOR ',') as modules, COALESCE(NULLIF(school_config.default_attendance, ''),'subject') as default_attendance FROM student LEFT JOIN student_group ON student.groupID = student_group.groupID LEFT JOIN classes ON classes.classID = student.classID LEFT JOIN section ON section.sectionID = student.sectionID LEFT JOIN school_config ON student.schoolID = school_config.schoolID LEFT JOIN school_subscription_history ON student.schoolID = school_subscription_history.schoolID AND school_subscription_history.is_active = 'Y' INNER JOIN modules ON FIND_IN_SET (modules.moduleID, school_subscription_history.modules) > 0 WHERE student.studentID = '$studentID'")->row();
            ...
            $student_detail = $this->webservice_m->get_single_table('student',array('studentID'=>$studentID));
            $all_modules = $this->db->query("SELECT `module_name` FROM `modules`WHERE `is_active` = 'Y'")->result();



            /*if (count($check)){*/
             if (!empty($check)){
                $modules_new_array = array();
                $module_name = '';
                $check[0]->modules = '';
                foreach ($all_modules as $m) 
                {
                    array_push($modules_new_array, $m->module_name);
                }  

                if(!empty($modules_new_array)){
                    //$module_name = implode(",", $modules_new_array);
                 $check[0]->modules = implode(",", $modules_new_array);
             }
               /* if ($check[0]->schoolID == $student_detail->schoolID){
        ...
        public function teacher_home()
        {
            ...
            $check[] = $this->db->query("SELECT teacher.teacherID,teacher.school_code,teacher.username,teacher.is_active,teacher.`name`,CONCAT('".$path."',COALESCE(NULLIF(teacher.image, ''),'default.png')) as image,GROUP_CONCAT(modules.module_name ORDER BY modules.module_name SEPARATOR ',') as modules, COALESCE(NULLIF(school_config.default_attendance, ''),'subject') as default_attendance FROM teacher LEFT JOIN school_config ON teacher.schoolID = school_config.schoolID LEFT JOIN school_subscription_history ON teacher.schoolID = school_subscription_history.schoolID AND school_subscription_history.is_active = 'Y' INNER JOIN modules ON FIND_IN_SET (modules.moduleID, school_subscription_history.modules) > 0 WHERE teacher.teacherID = '$teacherID'")->row();
                ...
                $teacher_detail = $this->webservice_m->get_single_table('teacher',array('teacherID'=>$teacherID));
                $all_modules = $this->db->query("SELECT `module_name` FROM `modules`WHERE `is_active` = 'Y'")->result();
            ...
            /*   if (count($subscription)) {*/
                if (!empty($subscription)) {
                    ...
                    /*if (count($get_school_config) && $get_school->is_config_setup == "Y") {*/
                     if (!empty($get_school_config) && $get_school->is_config_setup == "Y") {
                    ...
                    $response[] = array('result'=>'success','name'=>$get_school->name, 'email'=>$get_school->email, 'subscription_status'=>'Y', 'is_config_setup'=>$school_configuration, 'default_attendance'=>$default_attendance, 'image'=>base_url('uploads/images/').$img, 'modules'=>$module_name,'class_config'=>$class_configuration,'is_approved'=>'Y');
                    echo json_encode($response);
                }
        ...
        //echo $this->db->last_query($parents);
                ...
                public function action_approval()
                {
                    ...
                    if ($role == 'parent'){
                       /* if (strtolower($action) == 'Y'){*/
                        if ($action == 'Y'){
                            ...
                            $this->db->update('student',array('is_active'=>$action));
                            $student_detail = $this->db->get_where('student',array('studentID'=>$ID))->row();
                            ...
                            $this->webservice_m->table_update($role,array('is_active'=>$action),array($column => $ID, 'schoolID'=>$schoolID));
                            $teacher_detail = $this->db->get_where('teacher',array('teacherID'=>$ID))->row();
                            $username = $teacher_detail->username;
                            $password = $teacher_detail->real_password;
                    ...
                    public function bulk_approval()
                    {
                        ...
                        public function check_student_subscription()
                        {
                            ...
                            if(!empty($get_student) && $get_student->subscription_status == 'Y')
                            {
        ...
        //echo $this->db->last_query($get_student);
...
public function get_classes_admin()
{
    $data = json_decode(file_get_contents('php://input'), true);
    $schoolID = $data['schoolID'];
    //$classes = $this->webservice_m->get_all_data_where('admin_class',array('is_active'=>'Y'));
...
public function get_section()
{
    ...
    //$sections = $this->webservice_m->get_all_data_where('admin_section',array('class_id'=>$classID,'is_active'=>'Y'));
    $section = [];
...
public function view_student_attendance()
{
    ...
    $alldates = array();
    $check = $this->db->get_where('student_attendance',array('studentID'=>$studentID,'monthyear'=>$month_year))->row();
        //$check = $this->webservice_m->check_attendance(array('studentID'=>$studentID, 'subjectID'=>$subjectID, 'monthyear'=>$month_year));
        //echo $this->db->last_query($check);
...
public function view_teacher_attendance()
{
    ...
    //     $classID = $data['classID'];
    //     $sectionID = $data['sectionID'];
    //     $get_subjects = $this->webservice_m->get_all_data_where('admin_subject',array('classID'=>$classID,'is_active'=>'Y'));
    //     $subjects = array();
    //     foreach($get_subjects as $gs)
    //     {
    //         $subjects[] = array(
    //             'subjectID' => $gs->subID,
    //             'subject_name' => $gs->subject,
    //             'subject_code' => '',
    //             'type' => '',
    //             'syllabus' => '' 
    //         );
...
public function add_event()
{
    ...
    public function routine()
    {
        ...
        $this->db->insert('new',array('data'=>json_encode($data)));
        ...
        else if (strtolower($data['role']) == "student" || strtolower($data['role']) == "parent")
        {
        ...
        else if (strtolower($data['role']) == "teacher")
        {
            $schoolID = $data['schoolID'];
            $teacherID = $data['teacherID'];
            $day = strtoupper($data['day']);
                            ...
                            if (!empty($sectionID)){
                                ...
                                if ($group == 'student') {
                                            ...
                                            if (array_key_exists('receiverID',$data)){
                                                ...
                                                if (array_key_exists('sender_role',$data)){
                                                    ...
                                                    if (array_key_exists('receiver_role',$data)){
                                                        ...
                                                        $senderID = $data['senderID'];
                                                        $sender_role =  strtolower($data['sender_role']);
                                                        $receiverID = $data['receiverID'];
                                                        $receiver_role = $data['receiver_role'];
                                    ...
                                    public function change_password()
                                    {
                                        ...
                                        $real_password = $data['new_password'];
                                        if (strtolower($role)=='school'){
                                            $table = 'school_registration';
                                            $column = 'schoolID';
                                        }else{
                                        ...
                                        /*if (count($check)){*/
                                            ...
                                            if ($check){
                                               ...
                                               if($role == 'teacher'){
                                                $update['real_password'] = $real_password;
                                            }
...
//    }
    ...
    //End Batch
        ...
        //$quiz_bank = $this->webservice_m->get_all_data_where('quiz_bank',array('quiz_level'=>$topicID));
                    ...
                    //         //$this->db->select("`chapterID`, `chapter_name`, `is_active`");
                    //     $chapters = $this->db->get_where('admin_chapter',array('subjectID'=>$subjectID, 'is_active'=>'Y'))->result();
                    //     if(!empty($chapters))
                    //     {
                    //         $response[] =array('result'=>'success','chapters'=>$chapters);
                    //     } else {
                    //         $response[] = array('result'=>'failure','message'=>'No Chapter Available');
                    //     }
                    ...
                    // }
                        ...
                        public function chapters_list()
                        {
                            ...
                            $this->db->select("`chapterID`, `chapter_name`, `is_active`");
                            $chapters = $this->db->get_where('chapters',array('subjectID'=>$subjectID, 'is_active'=>'Y'))->result();
                            if(!empty($chapters))
                            {
                                $response[] =array('result'=>'success','chapters'=>$chapters);
                            } else {
                        ...
                        public function get_content_chapterwise()
                        {
                            $data = json_decode(file_get_contents('php://input'), true);
                            ...
                            $content = array();
                            //$path = base_url('uploads/contents/');
                            $path = 'https://admin.classplx.com/uploads/contents/';
                          //$this->db->select("`type`, `title`, `hls`, `is_active`");


                            $admin_class = $this->db->get_where('classes',array('classID'=>$classID,'schoolID'=>$schoolID))->row();

                                                   //echo $this->db->last_query();


                            $admin_class_id = $admin_class->admin_class_id;

                            $sections = $this->db->get_where('section',array('schoolID'=>$schoolID,'classID'=>$classID,'sectionID'=>$sectionID))->row();
                            $admin_section_id = $sections->admin_section_id;

                            $admin_subject = $this->db->get_where('subject',array('schoolID'=>$schoolID,'classID'=>$classID,'sectionID'=>$sectionID,'subjectID'=>$subjectID))->row();
                            $admin_subject_id = $admin_subject->admin_subject_id;

                            $chapters = $this->db->get_where('chapters',array('schoolID'=>$schoolID,'classID'=>$classID,'sectionID'=>$sectionID,'subjectID'=>$subjectID,'chapterID'=>$chapterID))->row();
                            $chapters_id = $chapters->admin_chapter_id;


                            // $contents = $this->db->get_where('admin_content',array('classID'=>$classID,'subjectID'=>$subjectID,'chapterID'=>$chapterID,'is_active'=>'Y'))->result();



                            $contents = $this->db->get_where('admin_content',array('classID'=>$admin_class_id,'section_id'=>$admin_section_id,'subjectID'=>$admin_subject_id,'chapterID'=>$chapters_id,'is_active'=>'Y'))->result();
                    ...
                    $subject_wise_content = $this->db->get_where('subject_wise_content',array('schoolID'=>$schoolID,'classID'=>$classID,'subjectID'=>$subjectID,'chapterID'=>$chapterID,'is_active'=>'Y'))->result();
                    if(!empty($subject_wise_content)){
                        foreach($subject_wise_content as $c)
                        {
                           if ($c->type == 'pdf') {
                            $c->hls =   $path.$c->hls; 
                            $pdf[] = array(
                ...
                if(!empty($pdf) || !empty($video))
                {
                    $response[] = array('result'=>'success','pdf'=>$pdf,'video'=>$video);
                } else {
                    $response[] = array('result'=>'failure','message'=>'No Content Available');
                }
            ...
            public function get_content_chapterwise_old()
            {
                $data = json_decode(file_get_contents('php://input'), true);
                $schoolID = $data['schoolID'];
                $classID = $data['classID'];
                ...
                $path = base_url('uploads/contents/');
                $this->db->select(" `type`, `title`, `hls`, `is_active`");
                $contents = $this->db->get_where('subject_wise_content',array('schoolID'=>$schoolID,'classID'=>$classID,'sectionID'=>$sectionID,'subjectID'=>$subjectID,'chapterID'=>$chapterID,'is_active'=>'Y'))->result();
        ...
        //@@  exam list api with pagination funcation by  pravas chandra nayak date: 21-12-2021 @@// 

        public function exam_list()
        {
            $data_json = json_decode(file_get_contents('php://input'), true);
            $student_id           = isset($data_json['student_id']) ? $data_json['student_id'] : "";
            $pageno               = isset($data_json['pageno']) ? $data_json['pageno'] : "";
            if ($student_id != ''){
                $check_student_id = $this->db->get_where('student',array('studentID' => $student_id))->result();
                if (!empty($check_student_id)) {
                    $student_data = $this->db->get_where('student',array('studentID' => $student_id))->row();
                    $schoolID   = $student_data->schoolID;
                    $classID    = $student_data->classID;
                    $sectionID  = $student_data->sectionID;
                    $exam_section_group = $this->db->get_where('exam_section_group',array('section_id' => $sectionID))->result();
                    $exam_id_array = array();
                    foreach ($exam_section_group as $key) {
                     array_push($exam_id_array, $key->exam_ids); 
                 }
                 if (!empty($exam_id_array)) {
                    $this->db->where_in('id',$exam_id_array);
                    $condition['school_id'] = $schoolID;
                    $condition['class_id'] = $classID;
                    $results_per_page = '10';
                    $pagination = $this->pagination($pageno,'exam_section',$condition,$results_per_page);

                    $this->db->where_in('id',$exam_id_array);
                    $this->db->limit($pagination['limit'],$pagination['offset']);
                    $exam_section = $this->db->get_where('exam_section',array('school_id' => $schoolID,'class_id' => $classID))->result();
           // or 
           //  $this->db->select('exam_section.*');    
           //  $this->db->from('exam_section');
           //  $this->db->join('exam_section_group', 'exam_section.id = exam_section_group.exam_ids');
           //  $this->db->where('exam_section.school_id',$schoolID);
           //  $this->db->where('exam_section.class_id',$classID);
           //  $this->db->where('exam_section_group.section_id',$sectionID);
           // // $this->db->group_by('exam_section_group.exam_ids'); 
           //  $exam_section = $this->db->get()->result();
           //   echo $this->db->last_query();
             //echo $this->db->last_query();
                    $list = array();
                    foreach ($exam_section as $data1) {
                        $check_answers = $this->db->get_where('answers',array('user_id'=>$student_id,'exam_id'=>$data1->id))->result();
                        $data=array(
                            'exam_id'                                 =>$data1->id,
                            ...
                            'time'                                    =>$data1->time,
                            'marks'                                   =>$data1->marks,
                            'negetive_marks'                          =>$data1->negetive_marks,
                            'no_of_question'                          =>$data1->no_of_question,
                            'description'                             =>$data1->description,
                            'start_date'                              =>$data1->start_date,
                            'end_date'                                =>$data1->end_date,
                            'status'                                  =>$data1->status,
                            'is_submit'                               =>!empty($check_answers) ? "YES" : "NO",

                        );
                        array_push($list, $data);
                    }
                    if ($list) {
                        echo json_encode(array('result'=>'success','message'=>'Successfully.' ,'total_rows'=>$pagination['total_rows'], 'data'=>$list));
                    }else{
                        echo json_encode(array('result'=>'success','message'=>'Sorry! Try again, Your request not completed.','total_rows'=>0, 'data'=>$list));
                    }
                }else{
                    echo json_encode(array('result'=>'failure','message'=>'examp not found !','total_rows'=>0));
                }
            }else{
                echo json_encode(array('result'=>'failure','message'=>'invalide student_id','total_rows'=>0));
            }
        }else{
            echo json_encode(array('result'=>'failure','message'=>'student_id required','total_rows'=>0));
        }
    }


    public function pagination($page,$table,$condition,$results_per_page){
        $total_rows = $this->db->get_where($table,$condition)->result();
        $total_rows = count($total_rows); 
        $number_of_result = $total_rows;  
        $number_of_page = ceil ($number_of_result / $results_per_page);  
        $page_first_result = ($page-1) * $results_per_page;  
        $data['total_rows']=$number_of_page;
        $data['offset']=$page_first_result;
        $data['limit'] =$results_per_page;
        $data['total_count'] =$total_rows;
        return $data;
    }

        //@@  questions list api by  pravas chandra nayak date: 21-12-2021 @@// 

    public function questions_list()
    {
        $data_json = json_decode(file_get_contents('php://input'), true);
        $student_id           = isset($data_json['student_id']) ? $data_json['student_id'] : "";
        $exam_id               = isset($data_json['exam_id']) ? $data_json['exam_id'] : "";
        if ($student_id != ''){
            if ($exam_id != ''){
                $check_student_id = $this->db->get_where('student',array('studentID' => $student_id))->result();
                if (!empty($check_student_id)) {
                    $exam_section = $this->db->get_where('questions',array('exam_id' => $exam_id,'status'=>1))->result();
                    $list = array();
                    foreach ($exam_section as $data1) {
                  //  $option = array('optionA'=>$data1->optionA,'optionB'=>$data1->optionB,'optionC'=>$data1->optionC,'optionD'=>$data1->optionD);
                        $data=array(
                            'questions_id'        =>$data1->id,
                            'questionName'        =>$data1->questionName,
                       // 'option'              =>$option,
                            'optionA'             =>$data1->optionA,
                            'optionB'             =>$data1->optionB,
                            'optionC'             =>$data1->optionC,
                            'optionD'             =>$data1->optionD,
                            'rightAnswer'         =>$data1->rightAnswer,
                        );
                        array_push($list, $data);
                    }
                    if ($list) {
                        echo json_encode(array('result'=>'success','message'=>'Successfully.', 'data'=>$list));
                    }else{
                        echo json_encode(array('result'=>'success','message'=>'Sorry! Try again, Your request not completed.', 'data'=>$list));
                    }
                }else{
                    echo json_encode(array('result'=>'failure','message'=>'invalide student_id'));
                }
            }else{
                echo json_encode(array('result'=>'failure','message'=>'student_id required'));
            }
        }else{
            echo json_encode(array('result'=>'failure','message'=>'exam_id required'));
        }
    }

        //@@  submit result api by  pravas chandra nayak date: 21-12-2021 @@// 
    ...
    public function submit_result()
    {
        ...
        $student_id           = isset($data_json['student_id']) ? $data_json['student_id'] : "";
        $exam_id              = isset($data_json['exam_id']) ? $data_json['exam_id'] : "";
        $submit_at            = isset($data_json['submit_at']) ? $data_json['submit_at'] : "";
        $submit_time            = isset($data_json['submit_time']) ? $data_json['submit_time'] : "";
        $submit_data            = isset($data_json['submit_data']) ? $data_json['submit_data'] : "";
        if ($student_id != ''){
            if ($submit_time != ''){
                if ($exam_id != ''){
                    if ($submit_at != ''){
                        if ($submit_data != ''){
                            ...
                            if (!empty($check_student_id)) {
                                $exam_section = $this->db->get_where('questions',array('exam_id' => $exam_id,'status'=>1))->result();
                                $list = array();
                            //$submit_data = '[{"question_id":"1","option_no":"1","is_report":"1","is_mark":"1"},{"question_id":"2","option_no":"1","is_report":"1","is_mark":"1"},{"question_id":"2","option_no":"0","is_report":"1","is_mark":"1"}]';
                                $this->db->insert('test_jeson',array('data'=>$submit_data));
                                $data2 = json_decode($submit_data);
                                $i =0;
                                foreach ($data2 as $da[$i]) {
                                    if (!empty($da[$i]->option_no)) {
                                        $question_data = $this->db->get_where('questions',array('id' =>$da[$i]->question_id))->row();
                                        $rightAnswer = $question_data->rightAnswer;
                                        if ($rightAnswer == $da[$i]->option_no) {
                                            $status = 1;
                                        }else{
                                           $status = 2; 
                                       }
                                 ...
                                 $answers =array(
                                    ...
                                    "exam_id"          => $exam_id,
                                    "question_id"      => $da[$i]->question_id,
                                    "option_no"        => !empty($da[$i]->option_no) ? $da[$i]->option_no : "0",
                                    "status"           => $status,
                                    "is_report"        => !empty($da[$i]->is_report) ? $da[$i]->is_report : "0",
                                    "is_mark"          => !empty($da[$i]->is_mark) ? $da[$i]->is_mark : "0",
                                    "submit_at"        => $submit_at,
                                );
                                 $q11 = $this->db->insert('answers',$answers);
                                 $i++;   
                             }
                             if ($q11) {
                                $correct_ans   = $this->db->get_where('answers',array('exam_id' => $exam_id,'user_id'=>$student_id,'status'=>1))->result();
                                $correct_ans   = !empty($correct_ans) ? count($correct_ans) : 0;
                                $wrong_ans     = $this->db->get_where('answers',array('exam_id' => $exam_id,'user_id'=>$student_id,'status'=>2))->result();
                                $wrong_ans     = !empty($wrong_ans) ? count($wrong_ans) : 0;
                                $skipped_ans   = $this->db->get_where('answers',array('exam_id' => $exam_id,'user_id'=>$student_id,'status'=>0))->result();
                                $skipped_ans   = !empty($skipped_ans) ? count($skipped_ans) : 0;
                                $questions     = $this->db->get_where('questions',array('exam_id'=>$exam_id))->result();
                                $questions_count   = !empty($questions) ? count($questions) : 0;
                                $exam_data     = $this->db->get_where('exam_section',array('id'=>$exam_id))->row();
                                $_mark         = $correct_ans * $exam_data->marks;
                                $negetive_marks= $wrong_ans * $exam_data->negetive_marks;
                                $t_mark        = $_mark - $negetive_marks;
                                $full_mark     = $questions_count * $exam_data->marks;
                                $ranking =array(
                                    "updated_at"       => "",
                                    "created_at"       => date("Y-m-d H:i:s"),
                                    "user_id"          => $student_id,
                                ...
                                $this->check_rank($exam_id);
                                echo json_encode(array('result'=>'success','message'=>'Submit Successfully.'));
                            }else{
                                echo json_encode(array('result'=>'success','message'=>'Sorry! Try again, Your request not completed.'));
                            }
                        }else{
                            echo json_encode(array('result'=>'failure','message'=>'invalide student_id'));
                        }
                    }else{
                        echo json_encode(array('result'=>'failure','message'=>'submit_data required'));
                    }
                }else{
                    echo json_encode(array('result'=>'failure','message'=>'submit_at required'));
                }
            }else{
                echo json_encode(array('result'=>'failure','message'=>'submit_time required'));
            }
        }else{
            echo json_encode(array('result'=>'failure','message'=>'student_id required'));
        }
 ...
 //@@  check rank funcation for submit_result api by  pravas chandra nayak date: 21-12-2021 @@// 


public function check_rank($exam_id){
    $this->db->order_by("marks", "desc");
    $this->db->order_by("time_taken");
    $ranking = $this->db->get_where('ranking',array('status' =>1,'exam_id'=>$exam_id))->result();
    //echo $this->db->last_query();
    $i = 0;
    foreach ($ranking as $key) {
        $i++;
        $this->db->where(array('id'=>$key->id));
        $this->db->update('ranking',array('rank'=>$i,'updated_at'=>date("Y-m-d H:i:s")));
    }
}

 //@@  result list api by  pravas chandra nayak date: 21-12-2021 @@// 
...
public function result()
{
    ...
    if ($student_id != ''){
        if ($exam_id != ''){
            $check_student_id = $this->db->get_where('student',array('studentID' => $student_id))->result();
            if (!empty($check_student_id)) {
                $ranking = $this->db->get_where('ranking',array('exam_id' => $exam_id,'user_id'=>$student_id))->row();
                $exam_section = $this->db->get_where('exam_section',array('id' => $exam_id))->row();
                $data=array(
                    'correct_ans'        =>$ranking->correct_ans,
                    'wrong_ans'          =>$ranking->wrong_ans,
                    'skipped_ans'        =>$ranking->skipped_ans,
                    'time_taken'         =>$ranking->time_taken,
                    'rank'               =>$ranking->rank,
                    'full_mark'          =>$ranking->full_mark,
                    'marks'              =>$ranking->marks,
                    'status'             =>$ranking->status,
                    'created_at'         =>$ranking->created_at,
                );
                if ($data) {
                    echo json_encode(array('result'=>'success','message'=>'Successfully.', 'data'=>$data));
                }else{
                    echo json_encode(array('result'=>'success','message'=>'Sorry! Try again, Your request not completed.', 'data'=>$list));
                }
            }else{
...
public function live_classes()
{
    $data_json = json_decode(file_get_contents('php://input'), true);
    $student_id           = isset($data_json['student_id']) ? $data_json['student_id'] : "";
    $role           = isset($data_json['role']) ? $data_json['role'] : "";

    if($role == 'Teacher'){
        $check_teacher_id = $this->db->get_where('teacher',array('teacherID' => $student_id))->row();
        if(!empty($check_teacher_id)){
            $live_classes = $this->db->get_where('live_classes',array('teacher_id'=>$student_id))->result();
            //print_r($live_classes);
            $list = array();
            foreach ($live_classes as $data1) {
                $today = date("Y-m-d");

                if($data1->start_date <= $today){
                    if($data1->end_date >= $today){
                        if($data1->start_time <= date('H:i:s')){
                            if($data1->end_time >= date('H:i:s')){
            ...
            echo json_encode(array('result'=>'failure','message'=>'invalide Teacher'));
    ...
    if($role == 'student' || $role == 'Student'){
     if ($student_id != ''){
        ...
        if (!empty($check_student_id)) {
            ...
            $live_classes = $this->db->get_where('live_classes',array('school_id' => $schoolID,'class_id'=>$classID,'section_id'=>$sectionID))->result();
           // echo $this->db->last_query();
            $list = array();
            foreach ($live_classes as $data1) {
                $today = date("Y-m-d");
...
public function get_class_list(){
    ...
    if(!empty($classes)){
        ...
        echo json_encode(array('result'=>'success','message'=>'Successfully','classes'=>$classes));

    }else{
        echo json_encode(array('result'=>'failure','message'=>'NO data FOund','data'=>null));
...
public function get_section_list(){
    $data_json = json_decode(file_get_contents('php://input'), true);
    $class_id = isset($data_json['class_id']) ? $data_json['class_id'] : "";
    if($class_id == ''){
        echo json_encode(array('result'=>'failure','message'=>'classID Required','data'=>null));
    }else{
        $sections = $this->db->get_where('admin_section',array('class_id'=>$class_id,'status' => 1))->result();
        if(!empty($sections)){
            foreach($sections as $section){
            }
            echo json_encode(array('result'=>'success','message'=>'Successfully','sections'=>$sections));
        }else{
            echo json_encode(array('result'=>'success','message'=>'NO data FOund','data'=>null));
...
public function insert_section(){
    ...
    if(!empty($classes)){
        foreach($classes as $class){
            $dbArr = [];
            $dbArr['class_id'] = $class->classid;
            $dbArr['name'] = 'D';
            $dbArr['status'] = 1;

            $this->db->insert('admin_section',$dbArr);
