...
class Dashboard extends MY_Controller
{
    ...
    public function index(){
        if ($_SESSION['role'] == "school" || $_SESSION['role']=="teacher" || $_SESSION['role'] == "other_staff") {
            if ($_SESSION['role'] == "school"){
                $schoolID = $_SESSION['loginUserID'];

            }elseif ($_SESSION['role'] == "other_staff"){
                $schoolID = $_SESSION['loginUserID'];
            }
    ...
    public function my_profile()
    {
        if ($_SESSION['role'] == "school" || $_SESSION['role']=="teacher" || $_SESSION['role'] == "other_staff") {
            ...
            elseif (strtolower($_SESSION['role']) == "other_staff"){
                $table = 'other_staff';
                $column = 'staffID';
                $userID = $_SESSION['loginUserID'];
               // echo $userID;
                $this->db->select("other_staff.school_code,CONCAT('".$path."',COALESCE(NULLIF(other_staff.image, ''),'default.png')) as image,other_staff.username,other_staff.address,other_staff.name,other_staff.phone,other_staff.email,school_registration.address as school_address,school_config.school_name,school_config.principal_name,school_config.email as school_email,school_config.phone as school_phone,school_config.school_prefix");
                $this->db->join('school_config','other_staff.schoolID = school_config.schoolID');
                $this->db->join('school_registration','other_staff.schoolID = school_registration.schoolID');
                $this->data['profile'] = $this->db->get_where('other_staff',array('other_staff.staffID'=>$userID))->row();
                $this->data['role'] = "Other Staff";
            }
