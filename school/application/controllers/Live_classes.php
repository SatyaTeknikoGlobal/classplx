...
class Live_classes extends MY_Controller
{
        ...
        if($_SESSION['role'] == "school" || $_SESSION['role'] == "teacher")
        {
            ...
            if ($_SESSION['role'] == "school") {
              $schoolID = $_SESSION['loginUserID'];
                ...
                $teacherID = $_SESSION['loginUserID'];
                $teacher = $this->db->get_where('teacher', array('teacherID'=>$teacherID))->row();
            ...
            if ($_POST) {
                 $details = array(
                  ...
                  $this->db->insert('live_classes',$details);

                  redirect(base_url('live_classes'));
            }else{
                //$schoolID = $_SESSION['loginUserID'];
            ...
            if ($_SESSION['role'] == "school") {
              $schoolID = $_SESSION['loginUserID'];
                ...
                $class_details = $this->db->query("SELECT `classID`,`class` FROM `classes` WHERE  `schoolID` = '$schoolID' ")->result();
                $teacher_details = $this->db->query("SELECT `teacherID`,`name` FROM `teacher` WHERE  `schoolID` = '$schoolID' ")->result();
                $live_classes = $this->db->query("SELECT `id`,`title`,`start_date`,`end_date`,`start_time`,`end_time` FROM `live_classes` WHERE  `school_id` = '$schoolID' AND `is_delete` = '0' ")->result();
                //echo $this->db->last_query();

                $this->data['school_id'] = $schoolID;
                $this->data['class_details'] = $class_details;
                $this->data['teacher_details'] = $teacher_details;               
                $this->data['live_classes'] =  $live_classes;
                $this->data['title'] = "Live Classes";
                $this->data['subview'] = "live_classes/index";
    ...
    {   
        if($_POST)
        {
            ...
            $sectionID = $this->input->post('section_id');
             $subject = $this->db->get_where('subject', array('classID'=>$classID , 'schoolID'=>$schoolID, 'sectionID' => $sectionID))->result();
                ...
                if ($_POST){
                   ...
                   $details = array(
                    ...
                    'end_time' => $this->input->post('end_time'),
                     ...
                     $this->db->where('id', $id);
                    $this->db->update('live_classes', $details);                   
                   ...
                   //$schoolID = $_SESSION['loginUserID'];
                    ...
                    $live_classes = $this->db->get_where('live_classes',array('id'=>$id,'school_id'=>$schoolID))->row();
                    
                    $this->data['live_classes'] = $live_classes;
                    $this->data['title'] = "Edit Class";
                    $this->data['subview'] = "live_classes/edit";
                    $this->load->view("layout",$this->data);
                }
            }
        }
     ...
     public function delete($id = "")
    {
        if (($_SESSION['role']) == 'school' ||  $_SESSION['role'] == "teacher")
        {
            ...
            $details = array('school_id'=>$schoolID, 'is_delete' => 1);
...
//                 print_r($insert);
//                 //$this->home_m->insert_data('exam_section',$insert);
...
//                         &nbsp;<a class = "btn btn-primary" title="Exam Routine" href = "' . base_url("exam/upload_questions/$d") . '"><i class="fa fa-question-circle"></i></a>
